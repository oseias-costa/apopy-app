...
export const Products = () => {
  ...
  const [ openModal, setOpenModal ] = useState<React.SetStateAction<boolean>>(false)
  const addRef = useRef<HTMLButtonElement>(null)
  const DOMNode = useRef<HTMLDivElement>(null)
  const [ mutateState, setMutateState ] = useState<MutationStateObject>({
    _id: "", name: "", type: "create", edit: '' })
  
  const typeModal: typeModal = {
    category: <Category setOpenModal={setOpenModal} categoryState={mutateState}  setCategoryState={setMutateState} />,
    suplier: <Suplier setOpenModal={setOpenModal} mutateState={mutateState}  setMutateState={setMutateState} />
  }
  ...
  useEffect(() => {
    window.addEventListener('click', (e) => {
       if(e.target !== document.getElementById('dropMenuAdd') && e.target !== addRef.current ){
        setAddItem(false)
      }})
  }, [])
  ...
  return (
    <div>
      ...
      />
      <h1>Products</h1>
      <CategoryList setOpenModal={setOpenModal} setMutateState={setMutateState}  />
      <h2>Fornecedores</h2>
